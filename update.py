...
# 503/429 등 일시 오류일 때 기다리는 시간(초)
RETRY_DELAYS = [20, 45, 75]
        ...
        output.append(
            {
                ...
                "title": str(item.get("title", src["title"])).strip()
                or src["title"],
...
def build_prompt(candidates: list[dict]) -> str:
    ...
    previous_terms_text = (
        ", ".join(previous_terms) if previous_terms else "없음"
    )
...
- 처음에는 LLM, Token, Parameter, Prompt, Context Window, Transformer,
  Inference, Fine-tuning, Embedding, RAG, Agent, NPU, Quantization 같은
  자주 듣는 기초 개념을 우선한다.
- 뉴스에 나온 단어와 꼭 연결할 필요는 없다.
- 이미 배운 단어는 가능하면 반복하지 않는다.
- 지금까지 배운 단어: {previous_terms_text}
- term: 실제 용어 또는 약자
- full_name: 약자의 영어 풀네임. 약자가 아니면 대표 영문명
- korean: 자연스러운 한국어 뜻
- explanation: AI 문외한도 이해할 수 있게 2~3문장
- example: ChatGPT, 스마트폰, 앱 같은 일상적인 상황의 한 문장 예시
...
def try_model_with_retries(
    ...
    for attempt in range(1, attempts + 1):
        try:
            print(
                f"[INFO] Gemini model={model_name} "
            ...
            last_error = e
            print(
                f"[WARNING] Gemini attempt failed: "
                f"model={model_name}, "
                f"attempt={attempt}/{attempts}, "
                f"{type(e).__name__}: {e}"
            )

            # 503/429/timeout 등 일시 오류가 아니면 같은 모델을
            # 계속 반복하지 않고 예비 모델로 넘어간다.
            if not is_temporary_error(e):
                print(
                    "[INFO] Non-temporary error detected; "
    ...
    raise RuntimeError(
        f"{model_name}에서 알 수 없는 오류가 발생했습니다."
    )


def summarize_with_gemini(candidates: list[dict]):
    ...
    models_to_try = [
        (MODEL, PRIMARY_ATTEMPTS),
    ]

    # 같은 이름이면 중복 호출하지 않는다.
    if FALLBACK_MODEL and FALLBACK_MODEL != MODEL:
        models_to_try.append(
            (FALLBACK_MODEL, FALLBACK_ATTEMPTS)
        )
    ...
    for model_name, attempts in models_to_try:
        try:
            ...
            print(
                f"[WARNING] Model exhausted: {model_name}. "
...
def fetch_usd_krw():
    ...
    try:
        ...
        print(
            f"[WARNING] Exchange rate fetch failed: "
            f"{type(e).__name__}: {e}"
        )
        return None
...
def fetch_usd_krw_history():
    ...
    try:
        ...
        print(
            f"[WARNING] Exchange history fetch failed: "
            f"{type(e).__name__}: {e}"
        )
        return []
    ...
    if HISTORY_INDEX.exists():
        try:
            ...
            if (
                ...
                and isinstance(existing.get("dates"), list)
            ):
...
def preserve_last_brief_after_failure(error: Exception):
    print(
        f"[WARNING] Gemini summary failed after retries: "
        f"{type(error).__name__}: {error}"
    )
    print(
        "[INFO] Keeping the last successful Korean brief. "
        "news.json/history will NOT be overwritten."
    )

    if not OUTPUT.exists():
        raise RuntimeError(
            "Gemini 요약에 실패했고 보존할 기존 한국어 "
            "브리핑도 없습니다."
        ) from error
    ...
    if exchange_rate_history:
        existing["exchange_rate_history"] = (
    ...
    existing["update_status"] = {
        "ok": False,
        "message": (
            "오늘 AI 요약 업데이트에 실패해 마지막 정상 "
            "한국어 브리핑을 표시하고 있습니다."
        ),
        "failed_at": datetime.now(KST).strftime(
            "%Y-%m-%d %H:%M KST"
        ),
        "error": str(error)[:500],
    }
    ...
    print(
        "[INFO] Updated exchange rate only; "
        "preserved the last successful AI brief."
    )
...
def main():
    ...
    if len(candidates) < 5:
        raise RuntimeError(
            f"RSS 기사 수집량이 너무 적습니다: "
            f"{len(candidates)}개"
        )

    try:
        result, used_model = summarize_with_gemini(
            candidates
        )

    except Exception as e:
        preserve_last_brief_after_failure(e)
        return
    ...
    final = {
        ...
        "generation_mode": (
            f"RSS + Gemini summary ({used_model})"
        ),
        "update_status": {
            "ok": True,
    ...
    print(
        f"Mode: RSS + Gemini summary ({used_model})"
    )
    print(f"Collected candidates: {len(candidates)}")
    print(
        f"Published issues: {len(final['issues'])}"
    )
